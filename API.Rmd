...
In this tutorial today we will use APIs to gather data from PubMed, and at the end I'll do a demo of using Twitter's API. For the PubMed portion of this tutorial we are going to use Dr. Aaron Gowins post on datascience+.com as a guide and outline [datascienceplus.com]. Here's a link to the post (https://datascienceplus.com/search-pubmed-with-rismed/). 



## Section III. Connecting and Querying APIs in R

Today we are using APIs to query data from databases. PubMed is the search engine the connects us to and allows us to query the MEDLINE database mainly composed of life science and biomedical topics (https://www.ncbi.nlm.nih.gov/pubmed/). "The United States National Library of Medicine at the National Institutes of Health maintain the database" (https://www.ncbi.nlm.nih.gov/pubmed/).
...
knitr::opts_chunk$set(echo = TRUE)
 install.packages(c("RISmed", "qdap", "jsonlite", "purrr", "data.table"), repos="http://cran.us.r-project.org")
```
...
"The EUtilsSummary function helps narrow a search query and will indicate how much data is available under the querying criteria. This is an important steps as it allows your to do some exploratory work with downloading the actual data." (https://amunategui.github.io/pubmed-query/)
...
"In order not to overload the E-utility servers, NCBI recommends that users post no more than three URL requests per second and limit large jobs to either weekends or between 9:00 PM and 5:00 AM Eastern time during weekdays. Failure to comply with this policy may result in an IP address being blocked from accessing NCBI." <-- from RIS Help EUtilsGet
...
res <- EUtilsSummary(query_str, db="pubmed", retmax=14000, mindate=2010, maxdate=2020)
# change to 2000 
QueryCount(res)
summary(res)
...
@Amunategui, M. (n.d.). Getting PubMed Medical Text with R and Package {RISmed}. Retrieved May 12, 2020, from https://amunategui.github.io/pubmed-query/

@Gowins, A. (2017, April 28). How to Search PubMed with RISmed package in R. Retrieved May 12, 2020, from https://datascienceplus.com/search-pubmed-with-rismed/

@Home - PubMed - NCBI. (n.d.). Retrieved May 12, 2020, from https://www.ncbi.nlm.nih.gov/pubmed
